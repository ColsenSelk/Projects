...
void io_battle(Character *aggressor, Character *defender)
{
  ...
  pokemon_Battle(); // starts pokemon battle
...
uint32_t move_pc_dir(uint32_t input, pair_t dest)
{
  ...
  check_tallgrass(dest); // checks to see if player is in tall grass, if they are then check_tallgrass has a chance to start a pokemon encounter
