...
void db_parse(bool print)
{
  ...
  i = (strlen(getenv("HOME")) +
       strlen("/Desktop/Assignments/Selk_Colsen.Assignment-1.07/pokedex/pokedex/data/csv/") + 1);
  prefix = (char *) malloc(i);
  strcpy(prefix, getenv("HOME"));
  strcat(prefix, "/Desktop/Assignments/Selk_Colsen.Assignment-1.07/pokedex/pokedex/data/csv/");
  */
  
  i = (strlen("pokedex/pokedex/data/csv/") + 1);
  prefix = (char *) malloc(i);
  strcpy(prefix, "pokedex/pokedex/data/csv/");
  ...
  for (i = 1; i <= 8; i++) {
    ...
    //tmp = next_token(NULL, ',');
    strncpy(stats[i].identifier, next_token(NULL, ','), 30);
  ...
  for (i = 1; i <= 6552; i++) {
    fgets(line, 800, f);
    tmp = next_token(line, ',');
    pokemon_stats[i].pokemon_id = *tmp ? atoi(tmp) : -1;
    tmp = next_token(NULL, ',');
  ...
  for (i = 1; i <= 1675; i++) {
    fgets(line, 800, f);
    tmp = next_token(line, ',');
    pokemon_types[i].pokemon_id = *tmp ? atoi(tmp) : -1;
    tmp = next_token(NULL, ',');
